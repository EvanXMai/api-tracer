...
export function mountSettingsDialog(): void {
  ...
  const formatBtn = document.getElementById('settings-format') as HTMLButtonElement
  ...
  apiPrefixDisplayEl = document.getElementById('api-prefix-display') as HTMLElement
  ...
  cancelEditBtn = document.getElementById('fn-cancel-edit') as HTMLButtonElement
  batchToggleBtn = document.getElementById('fn-batch-toggle') as HTMLButtonElement
  bulkDeleteBtn = document.getElementById('fn-bulk-delete') as HTMLButtonElement
...
async function onBulkDelete(): Promise<void> {
  const checkboxes = listEl.querySelectorAll<HTMLInputElement>('.fn-checkbox:checked')
...
function updateBulkDeleteState(): void {
  if (!batchMode) return
  const any = listEl.querySelectorAll<HTMLInputElement>('.fn-checkbox:checked').length > 0
